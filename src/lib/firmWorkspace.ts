...
export async function lookupInviteByToken(token: string): Promise<FirmInvite | null> {
  const { data } = await (db as any).rpc("get_firm_invite_by_token", { _token: token });
  const row = Array.isArray(data) ? data[0] : data;
  return (row as FirmInvite) || null;
}

export async function acceptInviteToken(token: string): Promise<{ firm_id: string } | null> {
  const { data: u } = await supabase.auth.getUser();
  if (!u.user) throw new Error("Not authenticated");
  const { data, error } = await (db as any).rpc("accept_firm_invite", { _token: token });
  if (error) throw error;
  if (!data) throw new Error("Invite could not be accepted");
  return { firm_id: data as string };
}
